...
//!
//! Used by the streaming `generate` export. Each `write_file` /
//! `create_dir_all` under `out_root` becomes an event delivered to the
//! callback synchronously; bytes are dropped on the wasm side after the
//! callback returns, so the previous `MemoryFs::drain_subtree` output
//! map is gone.
//!
//! What this does NOT bound:
//!   - **Input bundle**: source-side reads go through the inner
//!     `MemoryFs`, which holds the full project bundle for the call.
//!   - **Template render pass**: `template::fill` calls
//!     `render_in_memory` first and produces a `Vec<RenderedFile>`
//!     containing every rendered template string before the per-file
//!     write loop runs. `write_file` (the streaming sink) only fires
//!     inside that loop, so peak host memory during the template stage
//!     is the sum of every rendered template body, not one entry.
//!
//! The `copy` stage *does* stream one entry at a time — each file is
//! read and immediately written through the fs trait, so the callback
//! fires before the next file is read.
//!
//! Internal split:
