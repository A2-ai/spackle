...
# bundle-out exports. `ts/` is the published TypeScript package that
# wraps it. See `WASM.md` for architecture, `docs/ts/` for consumer docs.

# Build all three wasm-pack targets (nodejs, web, bundler) into
# `ts/pkg/<target>/`. One command; consumes the script.
build-wasm:
