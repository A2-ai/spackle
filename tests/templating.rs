...
fn error_malformed_syntax() {
    // Malformed syntax should surface as a per-file parse error from `fill`.
    ...
    let err = results.into_iter().next().unwrap().unwrap_err();
    assert!(
        matches!(err.kind, FileErrorKind::ErrorParsingTemplate(_)),
        "expected ErrorParsingTemplate, got {:?}",
        err.kind
    );
}
